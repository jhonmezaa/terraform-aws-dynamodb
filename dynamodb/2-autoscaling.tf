...
#
# 4 pairs of resources (target + policy):
# - Table read capacity autoscaling
# - Table write capacity autoscaling
# - Index read capacity autoscaling (per GSI via for_each)
# - Index write capacity autoscaling (per GSI via for_each)
################################################################################

################################################################################
# Table Read Autoscaling
################################################################################
...
resource "aws_appautoscaling_policy" "table_read_policy" {
  ...
  target_tracking_scaling_policy_configuration {
    ...
    scale_in_cooldown  = lookup(merge(var.autoscaling_defaults, var.autoscaling_read), "scale_in_cooldown", 0)
    scale_out_cooldown = lookup(merge(var.autoscaling_defaults, var.autoscaling_read), "scale_out_cooldown", 0)
    target_value       = lookup(merge(var.autoscaling_defaults, var.autoscaling_read), "target_value", 70)
  }
}

################################################################################
# Table Write Autoscaling
################################################################################
...
resource "aws_appautoscaling_policy" "table_write_policy" {
  ...
  target_tracking_scaling_policy_configuration {
    ...
    scale_in_cooldown  = lookup(merge(var.autoscaling_defaults, var.autoscaling_write), "scale_in_cooldown", 0)
    scale_out_cooldown = lookup(merge(var.autoscaling_defaults, var.autoscaling_write), "scale_out_cooldown", 0)
    target_value       = lookup(merge(var.autoscaling_defaults, var.autoscaling_write), "target_value", 70)
  }
}

################################################################################
# Index Read Autoscaling (per GSI)
################################################################################
...
resource "aws_appautoscaling_policy" "index_read_policy" {
  ...
  target_tracking_scaling_policy_configuration {
    ...
    scale_in_cooldown  = lookup(merge(var.autoscaling_defaults, each.value), "scale_in_cooldown", 0)
    scale_out_cooldown = lookup(merge(var.autoscaling_defaults, each.value), "scale_out_cooldown", 0)
    target_value       = lookup(merge(var.autoscaling_defaults, each.value), "target_value", 70)
  }
}

################################################################################
# Index Write Autoscaling (per GSI)
################################################################################
